<template>
	<div class="container" v-if="product.sku != undefined">
		...
		<div class="d-flex justify-between align-items-start">
			<!--  基本信息的左侧部分  -->
			<div class="spu-left">
				<img :src="`/api/img/${product.spu_image_current}`" class="w-100 rounded hand"/>
				<div class="mt-2 d-flex justify-between align-items-center text-large text-light text-bolder">
					<div class="hand" @click="image_left_clicked()">&lt;</div>
					<div class="spu-image-collection flex-grow-1 mx-2">
						<table border="0px" cellpadding="0px" cellspacing="4px" class="position-relative t-0"
							:style="{
								left : `-${product.spu_image_collection_left}px`
							}">
							<tr>
								<td v-for="spu_image of product.spu_image_list"
									:key="`td-${spu_image}`"
									@click="spu_image_list_clicked(spu_image)">
									<img :src="`/api/img/${spu_image}`" class="spu-image-thumbnail hand"
										:class="{
			...
			<!--  基本信息的右侧部分  -->
			<div class="spu-right">
				<div class="text-big text-gray text-bolder">{{product.sku.sku_name}}</div>
				<div class="mt-3 d-flex align-items-baseline">
					<div class="text-big text-red text-bolder mr-1">现价：</div>
					<div class="text-larger text-red text-bolder mr-4">￥{{product.sku.sku_price}}</div>
					<div class="text-sm text-muted text-bold mr-1">原价：</div>
					<div class="text-big text-muted text-bold text-decoration-linethrough">￥{{product.sku.sku_originalprice}}</div>
				</div>
				<div class="mt-3 text-muted text-bold">库存：{{product.sku.sku_quantity}}</div>
				<!--  商品的规格属性  -->
				<div class="mt-3 d-flex justify-between align-items-start"
					...
					<div class=" w-85 d-flex flex-wrap">
						<template v-for="attr_value of attr.attrValueList"
							:key="`attr-value-${attr_value.value_id}`">
							<div class="py-1 pl-1 pr-2 mr-2 mb-2 rounded  hand text-sm text-muted d-flex align-items-center"
								:class="{
									'border-red-1' : sku_contains_attr_value( attr.key_id , attr_value.value_id ),
									'border-gray-1' : !sku_contains_attr_value( attr.key_id , attr_value.value_id )
								}"
								v-if="attr_value.value_images.length > 0"
								@click="sku_attr_value_clicked( { key_id : attr.key_id , value_id : attr_value.value_id } )">
								<img :src="`/api/img/${attr_value.value_images[0]}`" class="spu-attr-value-image mr-1"/>
								<div>{{attr_value.value_name}}</div>
							</div>
							<div class="py-2 px-2 mr-2 mb-2 rounded hand text-sm text-muted"
								:class="{
									'border-red-1' : sku_contains_attr_value( attr.key_id , attr_value.value_id ),
									'border-gray-1' : !sku_contains_attr_value( attr.key_id , attr_value.value_id )
								}"
				...
				<div class="mt-3 d-flex" >
					<input type="number" min="1" v-model="product.spu_count" class="spu-count px-3 py-2 border-red-1 outline-0"/>
					<router-link to="/cart" @click="add_cart_clicked()" class="outline-0 border-0 py-2 px-3 bg-red bg-hover-light-red text-white text-decoration-none text-sm ml-3">加入购物车</router-link>
					<router-link to="/pay" class="outline-0 border-0 py-2 px-3 bg-red bg-hover-light-red text-white text-decoration-none text-sm ml-3">立即购买</router-link>
				</div>
			</div>
			<!--  /基本信息的右侧部分  -->
		</div>
