<template>
	<div class="container">
		...
		<div class="mt-3 p-3 rounded shadow" v-if="customer.selected_address != undefined">
			<div class="text-sm text-gray text-bold">收货人信息</div>
			<div class="mt-2 text-sm text-gray d-flex justify-between align-items-center">
		...
		<!--  收货信息  -->
		<!--  收货管理  -->
		<div class="mt-3 d-flex justify-between flex-wrap" v-show="customer.address_admin">
			<AddressEdit
				class="hand"
				v-for="address of customer.address_list"
				:key="`address-${address.uaddr_id}`"
				:address="address"
				@click="address_clicked(address)"></AddressEdit>
		...
		<div class="mt-4 text-xs text-muted">
			...
			<div class="d-flex justify-end mt-2">
				<div>订单总金额：</div>
				<div class="w-10 text-right">￥{{order.cart_total+order.distribution}}</div>
			</div>
			<div class="d-flex justify-end mt-2">
				<div>优惠金额：</div>
			...
			<div class="d-flex justify-end mt-3 align-items-baseline">
				...
				<div class="w-10 text-right text-big text-red text-bolder">￥{{order.cart_total+order.distribution-order.discount}}</div>
			</div>
			<div class="d-flex justify-end mt-3" v-if="customer.selected_address != undefined">
				<div>配送至：</div>
		...
		<!--  /结算清单  -->
		<div class="d-flex justify-end">
			<router-link to="/order_list" @click="add_order_clicked()" class="mt-3 text-decoration-none outline-0 text-white bg-red bg-hover-light-red py-2 px-3 rounded hand">提交订单</router-link>
		</div>
	</div>
</template>
