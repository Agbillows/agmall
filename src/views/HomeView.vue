<template>
  ...
  <div class="container d-flex justify-between">
    ...
    <div class="carousel-category shadow rounded-left py-2 position-relative">
      <!--  一级分类  -->
      <div
        class="text-xs text-gray bg-hover-light text-hover-red"
        v-for="cate_big of product.category_list"
        :key="`category-${cate_big.cate_id}`"
        @mouseover="category_hovered(cate_big)"
        @mouseout="category_outed()"
      >
        <div class="hand py-1 px-3">{{ cate_big.cate_name }}</div>
        <!--  分类右面板  -->
        <div
          class="category-panel bg-white shadow p-3"
          v-show="cate_big == product.category_current"
        >
          <!--  二级分类  -->
          <div
            class="text-xs d-flex align-items-start mb-1"
            v-for="cate_mid of cate_big.children"
            :key="`category-${cate_mid.cate_id}`"
          >
            <div
              class="text-gray text-hover-red text-bold hand w-10 text-right"
            >
              <span class="mr-2">{{ cate_mid.cate_name }}</span>
              <span> &gt; </span>
            </div>
            <div class="d-flex flex-wrap w-90 ml-2">
              ...
              <router-link
                to="/spu_list"
                class="text-muted text-hover-red text-decoration-none mx-2 mb-2"
                v-for="cate_small of cate_mid.children"
                :key="`category-${cate_small.cate_id}`"
                @click="category_clicked(cate_small)"
                >{{ cate_small.cate_name }}</router-link
              >
              <!--  /三级分类  -->
            </div>
          </div>
    ...
    <div class="carousel-image-list position-relative">
      <table
        class="carousel-table position-relative l-0 t-0 z-low"
        border="0px"
        cellpadding="0px"
        cellspacing="0px"
        :style="{
          left: `-${(website.carousel_current.carousel_id - 1) * 790}px`,
        }"
      >
        <tr>
          <td
            v-for="carousel of website.carousel_list"
      ...
      <div class="position-absolute b-4 r-5 z-high d-flex">
        <div
          class="p-2 rounded-circle hand ml-2"
          :class="{
            'bg-white':
              car_control.carousel_id == website.carousel_current.carousel_id,
            'bg-white-alpha':
              car_control.carousel_id != website.carousel_current.carousel_id,
          }"
          v-for="car_control of website.carousel_list"
          :key="`car_control-${car_control.carousel_id}`"
          @click="car_control_clicked(car_control)"
        ></div>
      </div>
    </div>
    <!--  /中间轮播图   -->
  ...
  <div
    ...
    v-for="(
      recommend_category, recommend_index
    ) of product.home_recommend_category_list"
    :key="`recommend-${recommend_category.cate_id}`"
  >
    <div class="d-flex">
      <div
        class="spu-list-title text-big text-bolder text-muted py-2 px-4 shadow"
      >
        {{ recommend_category.cate_name }} 专场
      </div>
    </div>

    <SpuList
      :spuList="product.home_recommend_spu_list[recommend_index]"
    ></SpuList>
  </div>
  <!--  /推荐商品 栏位  -->
</template>
...
export default {
  ...
  mounted() {
    ...
    if (sessionStorage.getItem("category_list")) {
      ...
      this.product.category_list = JSON.parse(
        sessionStorage.getItem("category_list")
      );
    } else {
      // 本地缓存 中 没有 分类列表
