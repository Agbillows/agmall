	...
	<div class="container px-3 py-1 rounded shadow text-sm text-muted">
		...
		<div class="d-flex align-items-start my-2">
			...
			<div class="w-90 d-flex flex-wrap">
				<div class="ml-3 hand"
					...
					@click="category_clicked(undefined)">全部</div>
				<div class="ml-3 hand"
					:class="{
					...
					@click="category_clicked(cate)">{{cate.cate_name}}</div>
			</div>
		</div>
		<!--  /商品分类 筛选面板   -->
		...
		<div class="d-flex align-items-start my-2"
			...
			<div class="w-90 d-flex flex-wrap">
				<div class="ml-3 hand"
					:class="{
						'text-red' : product.attr_selected_value_list[attr_index].value_id == undefined,
						'text-muted' : product.attr_selected_value_list[attr_index].value_id != undefined,
						'text-hover-red' : product.attr_selected_value_list[attr_index].value_id != undefined
					}"
					@click="attr_value_clicked( { index : attr_index , value_id : undefined } )">全部</div>
				<div class="ml-3 hand"
					:class="{
						'text-red' : product.attr_selected_value_list[attr_index].value_id == attr_value.id,
						'text-muted' : product.attr_selected_value_list[attr_index].value_id != attr_value.id,
						'text-hover-red' : product.attr_selected_value_list[attr_index].value_id != attr_value.id
					}"
					v-for="attr_value of attr.spuAttrValueList"
					:key="`attr-value-${attr_value.id}`"
					@click="attr_value_clicked( { index : attr_index , value_id : attr_value.id } )">{{attr_value.value_name}}</div>
			</div>
		</div>
		<!--  /商品筛选属性   -->
	</div>
	...
	mounted() {
		...
		if( this.product.cate_selected == undefined ){
			...
			if( sessionStorage.getItem("category") ){
				...
				this.product.cate_selected = JSON.parse( sessionStorage.getItem("category") )
